...
void Venda::imprimeEstoque() const {
    ...
    for(auto it = this->m_celulares.begin(); it != this->m_celulares.end(); it++){
        ...
        cout << it->_fabricante << " " << it->_modelo << ", " << it->_armazenamento << "GB" << ", " << it->_memoria << "GB RAM" << ", ";

        cout << std::fixed << std::setprecision(0) << (it->_peso)*1000 << " gramas" << ", ";
        ...
        cout << ", " << "R$ " << std::fixed << std::setprecision(2) << it->_valor << endl;
    }
}
